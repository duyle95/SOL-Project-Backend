...
exports.signupBasic = async (req, res, next) => {
    ...
    if (!company) {
        return res.status(422).send({ error: "We couldn't find any company with this code!" });
    }
...
exports.signupAdmin = async (req, res, next) => {
    ...
    const { user: {email, password}, company: { company_name: name } } = req.body;
    ...
    if (!name) {
        return res.status(422).send({ error: "You must provide your company name!"});
    }
    ...
    const user = new User({ email, password, role: "admin", _company: company._id });
